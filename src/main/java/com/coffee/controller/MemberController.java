...
@RestController // 해당 클래스는 웹 요청(from react)을 접수하여 처리해주는 컨트롤러 클래스입니다.
@RequiredArgsConstructor // final 키워드 또는 @NotNull 필드가 들어있는 식별자에 생성자를 통하여 값을 외부에서 주입해줍니다.
@RequestMapping("/member")
public class MemberController {
    private final MemberService memberService;

    @PostMapping("/signup") // SpringBoot 교안 p.144, p.179
    public ResponseEntity<?> signup(@Valid @RequestBody Member bean, BindingResult bindingResult) {
        // 회원가입을 위한 컨트롤러 메소드 -- bean은 유의미한 데이터를 뜻함
        // ResponseEntity : Http 응답 코드(숫자 형식)나, 적절한 메세지 등을 표현하기 위한 클래스
        // JSON : JavaScript Object Notation
        // @RequestBody : JSON 형태의 문자열을 자바의 객체 타입으로 변환해 줍니다.
        // @Valid : 입력 데이터에 대한 유효성 검사를 수행하는 어노테이션입니다, 사용 시 BindingResult가 반드시 사용되어야 합니다.
        // bindingResult : 유효성 검사 시 문제가 있으면 이 객체에 해당 예외 정보들이 포함됩니다.
        ...
        // 입력된 이메일을 이용하여 이메일 중복 여부 체크를 실행합니다.
    ...
    @PostMapping("/login") // <> Generic과 형변환 자바 이론문서 찾아볼 것
    public ResponseEntity<Map<String, Object>> login(@RequestBody Member bean) {
        // bean은 클라이언트가 기입한 로그인 정보를 담고 있는 객체
        System.out.println("클라이언트에서 로그인 요청 들어옴");
        System.out.println(bean);

        // member는 데이터 베이스에서 내가 조회한 회원 정보
        Member member = this.memberService.findByEmail(bean.getEmail());

        boolean isFound = false; // 회원이 발견되면 true로 바뀜 (flag 변수)

        if (member != null) { // 입력한 아이디가 존재함
