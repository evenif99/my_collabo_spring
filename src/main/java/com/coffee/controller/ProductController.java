...
public class ProductController {
    ...
    public ResponseEntity<?> insert(@RequestBody Product product) {
        ...
        String imageFileName = "product" + System.currentTimeMillis() + ".jpg";
        ...
        String imageData = product.getImage(); // Base64 인코딩 문자열(엄청 긴 문자열)

        try {
            // 파일 정보를 byte 단위로 변환하여 이미지를 복사합니다.
            FileOutputStream fos = new FileOutputStream(imageFile);

            // method chaining : '.'(점)을 연속적으로 찍어서 메소드를 계속 호출하는 것
            // split 메소드 :
            ...
            return ResponseEntity.ok(Map.of("message", "Product insert successful", "image", imageFileName));
    ...
    @GetMapping("/update/{id}") // 상품의 id 정보를 이용하여 해당 Product Bean 객체를 반환해 줍니다.
    public ResponseEntity<Product> getUpdate(@PathVariable Long id) {
        System.out.println("수정할 상품 번호" + id);

        Product product = this.productService.getProductById(id);

        if (product == null) { // 상품이 없으면 404(page not found) 응답과 함께 null을 반환
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);

        } else { // 해당 상품의 정보와 함께, 성공(200) 메세지를 반환합니다.
...
//        System.out.println(updatedProduct);
        ...
        if (findProduct.isEmpty()) { // 상품이 존재하지 않으면 404 응답 반환
            ...
            try {
                ...
                // 이미지가 의미 있는 문자열로 되어 있고, Base64 인코딩 형식이면 이미지 이름 변경합니다.
            ...
            } catch (Exception err) { // 오류 발생 시 500 응답 코드 반환
    ...
    public ResponseEntity<Product> detail(@PathVariable Long id){
        ...
        if(product == null){ // 404 NOT_FOUND 응답
